...
-  **Приемные родители** – если для объекта создана связь с каким-либо
   ...
   контейнера (подробнее см. `Управление связями объектов`_).
...
`Копирование объектов`_. |image64| **Перенести объект** – см. раздел
`Перемещение объектов`_. |image65| **Разорвать связь** – разорвать связь
объекта с приемным родителем (см. раздел `Удаление связи`_).
...
-  Кнопка **Редактировать объект** |image67| – для перехода в режим
   редактирования объекта (см. `Управление объектами в режиме редактирования`_).
...
настраиваются в редакторе категорий (см. `Создание редактирование категорий объектов`_). Для каждого поля в редакторе
...
-  **editor** – текстовое поле, аналогичное **textarea**. Отличие от
   **textarea** состоит в том, что при вводе текста в данное поле в
   ...
   форматирования текста (подробнее см. в разделе `Редактор форматируемого текста`_).
...
-  **CQL** – данный тип поля можно добавлять только в те категории,
   ...
   контейнеров. Поле предназначено для создания CQL-запроса (см. `CQL-запрос`_).
...
просмотра (см. `Управление объектами в режиме просмотра`_) и режим редактирования (см. `Управление объектами в режиме редактирования`_). В первом режиме поля заблокированы, во
...
маркерами разного цвета (см. раздел `Ввод и валидация значений`_).
...
команды контекстного меню `Просмотр/запуск объектов`_ в новой вкладке
...
это описано в разделе `Создание утилиты`_.
...
мультикатегориями (см. `Мультиплицируемая категория`_). Для создания нового блока полей нажмите на кнопку
...
подробной информации о данном объекте (см. `Просмотр подробной информации`_). Запрос имеет следующий
...
`Табличное представление объектов системы`_).
...
-  **Идентификатор свойства** – запрос, в котором указывается, данные
   ...
   ``id category`` – идентификатор категории (см. `Создание редактирование категорий объектов`_), в которой находится данное
...
Блок полей **Свойства утилиты** является мультикатегорией (см. `Мультиплицируемая категория`_). Для создания
...
выполните для этого контейнера команду контекстного меню **Выделить >Установить указатель текущего местоположения** (см. `Установка указателя текущего местоположения`_). Чтобы выполнить поиск по
...
-  **Редактировать** – предназначена для перехода в карточку объекта,
   ...
   режиме редактирования (см. `Управление объектами в режиме редактирования`_).
